...
<div class="container">
	<div class="row">
		<div class="page-header" style="margin: 0px;">
		  <h1>Outdoor Power Equipment <small>Add / Remove the outdoor power equipment</small></h1>
		</div>
	</div>
	</br>
	<div class="row">
		<form name="addOutdoorPower" class="form-horizontal myform" action="manage_outdoor_power_tools_action.php" method="POST" enctype="multipart/form-data" >
		  ...
		  <div class="form-group">
		    <label for="" class="col-sm-2 control-label">Outdoor Power Equipment Type</label>
		    <div class="col-sm-10">
		    	<?php
		    		$types = $a->getTypes();
		    		if (is_array($types) || is_object($types)){
		    			foreach ($types as $type) {
			    			echo "<div class='radio'>";
	  						...
	  						if($type['id'] == $product["outdoor_power_type_id"]) {
	  							echo " checked ";
	  						}
		...
		<form style="display: none;" class="deleteform" action="manage_outdoor_power_tools_action.php" method="POST"  >
			<input type='hidden' name='action_type' value='addProduct' />
		...
		<?php
			if (is_array($products) || is_object($products))
			{
				...
				foreach ($products as $result) {
					...
					$typeName = $a->getTypeById($result['outdoor_power_type_id']);

					echo "<td class=''>".$typeName["type_name_en"].": ".$typeName["type_name_th"]."</td>";
					echo "<td><a href='manage_outdoor_power_tools.php?product_id=".$result["id"]."' class='btn btn-info update-btn' value=''>View & Update</a>&nbsp;&nbsp;&nbsp;";
